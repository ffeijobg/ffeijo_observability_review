...
resource "null_resource" "install_argocd" {
  ...
  provisioner "local-exec" {
    ...
    command = "kubectl create namespace argocd --dry-run=client -o yaml | kubectl apply -f - && kubectl apply -n argocd -f https://raw.githubusercontent.com/argoproj/argo-cd/${var.argocd_version}/manifests/install.yaml && kubectl -n argocd rollout status deployment/argocd-server --timeout=180s && kubectl -n argocd rollout status deployment/argocd-repo-server --timeout=180s"
  }
}
